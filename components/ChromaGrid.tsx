...
const ChromaGrid: React.FC<ChromaGridProps> = ({
  ...
  return (
    <div
      ...
      className={`relative w-full h-full flex justify-center items-start ${className}`}
      style={{
        '--r': `${radius}px`,
        '--x': '50%',
        '--y': '50%'
    ...
    >
      {data.map((c, i) => (
        <article
          ...
          className="group relative flex flex-col w-[320px] h-[400px] rounded-[20px] overflow-hidden border-2 border-gray-700 bg-gray-800 transition-all duration-300 cursor-pointer hover:border-transparent"
          style={{
        ...
        >
          ...
          <footer className="relative z-10 p-4 text-white font-sans h-[120px] flex flex-col justify-between">
            <div className="flex justify-between items-start">
              <h3 className="m-0 text-[1.05rem] font-semibold leading-tight flex-1 pr-2">{c.title}</h3>
              {c.handle && <span className="text-[0.85rem] opacity-80 text-right whitespace-nowrap">{c.handle}</span>}
            </div>
            <div className="flex justify-between items-end mt-2">
              <p className="m-0 text-[0.85rem] opacity-85 leading-tight flex-1 pr-2">{c.subtitle}</p>
              {c.location && <span className="text-[0.85rem] opacity-85 text-right whitespace-nowrap">{c.location}</span>}
            </div>
          </footer>
        </article>
      ))}
